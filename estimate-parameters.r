...
# Optimise over cases.
Opt <- optim(c(beta,gamma,p), RSS.SEIRC, method = "L-BFGS-B", lower = lower, upper = upper)
Opt_par<-Opt$par
names(Opt_par)<-c("beta","gamma","p")
...
#Solve the ODEs for the 1st optimal parameter estimates: 
solution1<-as.data.frame(rk4(y = init, times = day, func = fn, parms = Opt_par))
colnames(solution1)<-c("time","S","E","I","R","C","D")
#Plot the resulting C vs the real cummulative cases
#plot(Cummulative_cases)
#lines(solution1$C,col="red")
...
#Now try to combine both to get the best fit for both C and D:
RSS <- function(parameters) {
  names(parameters) <- c("beta","gamma","p")
  out <- rk4(y = init, times = day, func = fn, parms = parameters)
  fit1 <- out[ ,6]
  fit2<-out[,7]
  RSS <- sum((Cummulative_cases - fit1)^2)+sum((Cummulative_deaths-fit2)^2)
  return(RSS)
}

# Optimising for the joint reducting in RSS.
Opt3 <- optim(c(beta,gamma,p), RSS, method = "L-BFGS-B", lower = lower, upper = upper)
Opt3_par<-Opt3$par
names(Opt3_par)<-c("beta","gamma","p")

# Solve with optimal parameters.
solution3<-as.data.frame(rk4(y = init, times = day, func = fn, parms = Opt3_par))
colnames(solution3)<-c("time","S","E","I","R","C","D")

# Plot cases and deaths.
2par(mfrow = c(1, 2))
plot(Cummulative_cases)
lines(solution3$C,col="red")
plot(Cummulative_deaths)
lines(solution3$D,col="red")
